/*
 * In order to simplify the access for reading the input
 * 
 * 
 * ADS1256_input.h:
...
// modified, because ads1256_input.h is only necessary in dac8532_ouput
/*
...
int  get_input(float adc_values[], int ch_num)
{
    uint8_t id;
  	int32_t adc[8];
	int32_t volt[8];
...
//		printf("\33[%dA", (int)ch_num);  
	...
	for (i = 0; i < ch_num; i++){
		...
		if(iTemp < 0){
			...
			sprintf(buffer1, "%ld.", iTemp/1000000);	
			sprintf(buffer2, "%03ld", (iTemp%1000000)/1000);
			sprintf(buffer3, "%03ld", iTemp%1000);	
			strcat(buffer1,buffer2);
			strcat(buffer1,buffer3);

			adc_values[i]= atof(myStr);
